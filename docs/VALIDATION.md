...
## Python 3.14 and stable dependency verification

On 2026-09-07, the project was moved to Python 3.14 with a `.python-version` pin of 3.14.7, matching the [current stable maintenance release](https://blog.python.org/2026/08/python-3147-31315/). PyPI metadata for every direct Python dependency and npm metadata for every direct frontend dependency were checked. All resolved versions were already current stable releases. Python minimum requirements now match that tested baseline, and uv explicitly excludes prereleases. The regenerated lockfile retains the same library versions while removing artifacts specific to older Python versions.

All 133 Python tests pass on CPython 3.14.7, along with Ruff lint and formatting for the Python 3.14 target. The source archive includes `.python-version`. An isolated wheel installation on 3.14.7 passed the CLI and authenticated localhost API/asset checks. Its synthetic demo produced 18 tasks and two experiments, returned the expected four-record filtered search, made zero provider calls and left final exposure unused. The upstream Starlette TestClient deprecation warning remains. Frontend source, dependencies and bundled assets did not change in this migration.
