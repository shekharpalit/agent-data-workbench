...
- Organize Python by responsibility: `data/` for ingestion and storage, `analysis/` for batch findings, `research/` for native investigations, `evaluation/` for tasks and experiments, `execution/` for target adapters, `exploration/` for search/clustering/lineage, `integrations/` for provider and export adapters, `workspace/` for project state, and `workbench/` for local application lifecycle. Put domain-independent utilities in `shared/`; it must not import domain packages. Keep root SDK exports in `__init__.py` and split modules at responsibility boundaries instead of adding flat root modules.
- Run the FastAPI app through standard Uvicorn entrypoints. Use FastAPI/Starlette middleware and BackgroundTasks instead of custom sockets, server wrappers, raw ASGI request parsing, or manually spawned HTTP job threads.
...
- Keep frontend source in `ui/`. Vite generates ignored `ui/dist/`; never commit compiled UI assets under the Python source tree. Build the UI before creating a Python package, and verify `uv build` when packaging changes.
